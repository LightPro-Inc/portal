(function(app){
	...
	editOrderProductCtrl.$inject = ['data', '$uibModalInstance', 'apiService', 'notificationService', 'utilityService', 'Guid', '$scope', '$timeout', '$uibModal'];
	function editOrderProductCtrl(data, $uibModalInstance, apiService, notificationService, utilityService, Guid, $scope, $timeout, $uibModal){
		var vm = this;		
		...
		function saveItem(){				
			...
			if(vm.isNewItem){									
				return apiService.post(String.format('/web/api/purchase-order/{0}/product', vm.orderId), vm.item,
						function(response){						
				...
				return apiService.put(String.format('/web/api/purchase-order/{0}/product/{1}', vm.orderId, vm.item.id), vm.item,
						function(response){
