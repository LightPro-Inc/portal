(function(app){
	...
	app.controller('pdvProductCategoryCtrl', pdvProductCategoryCtrl);
	
	pdvProductCategoryCtrl.$inject = ['apiService', '$uibModal', '$confirm', 'notificationService', '$state', '$previousState','$stateParams'];	
	function pdvProductCategoryCtrl(apiService, $uibModal, $confirm, notificationService, $state, $previousState, $stateParams){
		var vm = this;
		...
		function removeProduct(item) {
			...
			apiService.remove(String.format('/web/api/pdv/pdv/{0}/product-category/{1}', vm.pdvId, item.id), {}, 
					function(result){					
		...
		function addProduct(item) {
			...
			apiService.post(String.format('/web/api/pdv/pdv/{0}/product-category/{1}', vm.pdvId, item.id), {}, 
					function(result){					
		...
		function searchFreePdv(pageFreePdv){
			...
			apiService.get(String.format('/web/api/pdv/pdv/{0}/free-product-category/search', vm.pdvId), config, 
					function(result){					
		...
		function searchPdToSale(pagePdToSale){
			...
			vm.loadingDataPdToSale = true;
			apiService.get(String.format('/web/api/pdv/pdv/{0}/product-category-to-sale/search', vm.pdvId), config, 
					function(result){					
						vm.loadingDataPdToSale = false;
