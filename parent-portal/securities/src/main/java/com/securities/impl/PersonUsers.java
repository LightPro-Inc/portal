...
public class PersonUsers implements Persons {
	...
	public PersonUsers(final Base base, final Persons origin, final Membership membership){
		this.origin = origin;
	...
	public List<Person> find(int page, int pageSize, String filter) throws IOException {
		...
		String statement = String.format("SELECT %s FROM %s "
				+ "WHERE (concat(%s,' ', %s) ILIKE ?  OR concat(%s, ' ', %s) ILIKE ?) AND %s=? AND %s IN (SELECT %s FROM %s) "
				+ "ORDER BY %s DESC ", 
